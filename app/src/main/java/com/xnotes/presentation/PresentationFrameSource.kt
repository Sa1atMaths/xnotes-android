...
class PresentationFrameSource(
    ...
    private fun highlightsFor(page: Page): List<Stroke> {
        ...
        for (item in page.items) {
            if (item is Stroke && item.tool == Tool.HIGHLIGHTER && !state.isLiftedItem(item)) {
                out.add(Stroke(item.tool, item.config, ArrayList(item.samples), item.speedScale, item.straight))
            }
        }
        return out
    }
    ...
    private fun liveSnapshotFor(pageIndex: Int): Stroke? {
        ...
        val src = ls.second
        return Stroke(src.tool, src.config, ArrayList(src.samples), src.speedScale, src.straight)
    }
}
