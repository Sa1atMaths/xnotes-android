...
class SessionStoreTest {
    ...
    @Test fun roundTripsDocumentIdentityAndViewState() {
        ...
        SessionStore(dir, codec()).save(doc, zoom = 2.04, scrollX = 120.0, scrollY = 340.0, writeDocument = true)
    ...
    @Test fun viewStateRefreshKeepsExistingDocument() {
        val dir = tempDir()
        SessionStore(dir, codec()).save(Document.blank(3), zoom = 1.0, scrollX = 0.0, scrollY = 0.0, writeDocument = true)
        // A metadata-only update (writeDocument = false) must not lose the document.
        SessionStore(dir, codec()).save(Document.blank(9), zoom = 1.5, scrollX = 10.0, scrollY = 20.0, writeDocument = false)
